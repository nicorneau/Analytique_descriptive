...
cluster <- kmeans(profil[1:20, c("DepenseJeu", "DepensesAutres")], centers = 4)
# visualisation des résultats
plot(profil[1:20, c("DepenseJeu", "DepensesAutres")], col = (cluster$cluster),
     main = "Profils de joueurs selon le niveau de dépenses",
     xlab = "Dépenses de jeu", ylab = "Autres dépenses", pch = 20)
mtext("(segmentation à 4 groupes)")
```
...
Le premier segment (en vert) semble être des clients qui viennent au casino à des fins récréatives. Ils dépensent relativement peu et forment le groupe le plus nombreux. Le deuxième segment (en bleu) semble lui-aussi être présent dans une perspective récréative puisqu'il dépense de façon marquée, notamment dans les autres dépenses. Il semble toutefois plus fortuné que le premier segment. Le troisième segment (en rouge) parait être des joueurs sérieux ne dépensant qu'en jeu. Le dernier segment (en noir) est possiblement des joueurs professionnels ou de gens très fortunés dépensant des montants importants dans le jeu.
